...
public class FormErrorConverterImpl implements FormErrorConverter {
    ...
    @Override
    public void convertToFieldError(Class<? extends Annotation> validationAnnotationClass,
        ...
        if (!found) {
            throw new IllegalArgumentException("validationAnnotationClass not found for EnableFormErrorConversion annotation: "
                    + "validationAnnotationClass=" + validationAnnotationClass
                    + ", formInstance=" + formInstance.getClass());
        }
    }

    protected void processGlobalErrorConversion(Annotation validationAnnotation,
                                                EnableFormErrorConversion metaAnnotation,
                                                Object formInstance, BindingResult bindingResult) {
    ...
    protected void processFormFieldErrorConversion(Annotation validationAnnotation,
                                                   EnableFormErrorConversion metaAnnotation,
                                                   Object formInstance, BindingResult bindingResult) {
    ...
    protected void createFieldErrorForErrorCode(EnableFormErrorConversion metaAnnotation,
                                                FieldError fieldError, Object formInstance, BindingResult bindingResult) {
        ...
        bindingResult.addError(new FieldError(formName, fieldError.getField() + "."
                + fieldName, value, false, fieldError.getCodes(),
                fieldError.getArguments(), fieldError.getDefaultMessage()));
    }

    protected void createFieldErrorForErrorCode(EnableFormErrorConversion metaAnnotation,
                                                ObjectError objectError, Object formInstance, BindingResult bindingResult) {
        String fieldName = metaAnnotation.fieldReference();
        String formName = metaAnnotation.formName();
        Object value = formInstance instanceof Iterable<?> ? null : DataUtil.readProperty(formInstance, fieldName);
        bindingResult.addError(new FieldError(formName, fieldName, value, false, objectError.getCodes(),
